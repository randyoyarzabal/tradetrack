...
class CurrencyFormatter:
    ...
    def format_currency(
        ...
        show_symbol = show_symbol if show_symbol is not None else self.currency_config['show_symbol']
        decimal_places = decimal_places if decimal_places is not None else self.currency_config['decimal_places']
        colored_mode = colored_mode if colored_mode is not None else self.currency_config['colored_mode']
        negative_format = negative_format if negative_format is not None else self.currency_config['negative_format']
        ...
        if is_negative:
            if negative_format == "parentheses":
                formatted_value = f"({formatted_value[1:]})"  # Remove minus sign and add parentheses
